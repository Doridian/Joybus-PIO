...
static inline GCControllerState joybus_gc_read_state(JoybusPIOInstance instance, uint8_t payload[], int payload_len, int response_overhead) {
  ...
  state.valid = joybus_pio_transmit_receive(instance, payload, payload_len, (uint8_t*)&state, 8 + response_overhead) == 8 + response_overhead;
...
#define GC_NORMALIZE_ELEMENT(NAME) output.NAME = ((int)state.NAME) - ((int)origin.NAME)
